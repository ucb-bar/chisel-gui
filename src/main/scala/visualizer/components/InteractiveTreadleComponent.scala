...
class InteractiveTreadleComponent(dataModel: DataModel, displayModel: DisplayModel) extends GridPanel(2, 1) {
  ...
  val inputArea: GridPanel = new GridPanel(2, 1) {
    val pokeRow: FlowPanel = new FlowPanel {
      var signalsComboBox: ComboBox[String] = new ComboBox(TreadleController.pureSignalMapping.keys.toSeq)
      ...
      reactions += {
        case _: PureSignalsChanged =>
          signalsComboBox = new ComboBox(TreadleController.pureSignalMapping.keys.toSeq)
      ...
      val button = new Button(Action("Go") {
        try {
          ...
          TreadleController.tester match {
            case Some(t) =>
              t.step(n)
              dataModel.loadMoreValues()
              textArea.append(s"Step $n cycles\n")
            case None => // TODO: Notify that not connected to treadle
          }
        } catch {
          case _: NumberFormatException => // TODO: Notify that value is invalid
        }
